...
└── site/
    └── config.md  site identity, navigation, and browse dimensions
```
...
`config.md`. `/browse/` lists every configured dimension; `/browse/<slug>/`
...
in `src/content/site/config.md`'s frontmatter — only override what you
need. `config.md`'s body isn't read by the site — it's in-file
documentation for each field, meant to be edited alongside the rest of the
Obsidian vault.
...
- `src/content/site/config.md` — replace the `# ← replace` placeholders (title, author, url, social links).
